...
sub challenge( Int $number ) { (1..$number).map({ ʗ$_ }); }

#multi sub MAIN( Int $number ) { say 'The first ',$number,' Chowla numbers are: '; say challenge($number); }

#multi sub MAIN( 'challenge' ) { MAIN(20); }
