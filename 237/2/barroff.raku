...
sub maximise-greatness(@ints --> Int:D) {
    max(map({ greatness(@ints, $_) }, permutations(@ints)));
    #max(map({ greatness(@ints, $_) }, permutations(@ints).race)); # so JVM will work
}
...
#multi sub MAIN(*@ints where @ints.elems ≥ 1) { say maximise-greatness(@ints); }
