...
sub date_finder ( $string ) {
    ...
    say qq{Bad value: Century $century} if $century != 2 && $century != 1;
    say qq{Bad value: Month   $month} if $month < 1 || $month > 12;
    say qq{Bad value: Day     $day}   if $day < 1   || $day > 31;
    ...
    try {
        ...
        say "Bad value: Input is not a day ($string)";
    };
}
